...
let MontarKitFilasPage = class MontarKitFilasPage {
    ...
    submitForm() {
        ...
        submitBtn.addEventListener('click', () => {
            const previousData = JSON.parse(document.querySelector('#previous-data').textContent);
            const rowFields = [...document.querySelectorAll('.fila-field')];
            const data = {
                filas: rowFields.map((row) => ({
                    qntd_placas: +row.querySelector('[name="qntd_placas"]').value,
                    orientacao: row.querySelector('[name="orientacao"]').value
                })),
                ...previousData
            };
